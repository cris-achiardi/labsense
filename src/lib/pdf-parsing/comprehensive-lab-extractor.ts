...
import {
	createHealthMarkerLookup,
	type HealthMarkerMapping,
} from './spanish-health-markers';
...
function extractLabNamesAndResults(
	...
	for (const exactLabName of exactLabNames) {
		...
		for (
			...
			while ((match = pattern.exec(text)) !== null) {
				...
				if (labResult) {
					console.log(
						`✅ EXACT match: ${labResult.examen} = ${labResult.resultado}`
					);
	...
	console.log(
		`🎯 EXACT extraction found ${results.length} LAB NAME + RESULT pairs`
	);
	return results;
}
...
function createExactLabResult(
	...
	const severityData =
		typeof parsedResultado === 'number' && format.normalRange
			? calculateClinicalSeverity(
					parsedResultado,
					format.normalRange,
					format.unit || ''
				)
...
function parseLipidProfile(
	...
	for (const labName of lipidLabs) {
		...
		const pattern = new RegExp(
			`(${escapedLabName})(\\d+(?:,\\d+)?(?:\\.\\d+)?)\\(([^)]+)\\)([^A-ZÁÉÍÓÚÑ]*?)(?=COLESTEROL|CALCULO|$)`,
			'g'
		);
		...
		if (match) {
			...
			if (labResult) {
				...
				console.log(
					`✅ Lipid profile: ${labResult.examen} = ${labResult.resultado}`
				);
				results.push(labResult);
			}
		}
	...
	for (const labName of hemogramLabs) {
		...
		if (match) {
			...
			if (labResult) {
				...
				console.log(
					`✅ Hemogram: ${labResult.examen} = ${labResult.resultado}`
				);
				results.push(labResult);
			}
		}
...
function segmentBySampleType(text: string): SampleTypeBlock[] {
	...
	console.log(
		`🔍 Found ${Math.floor(segments.length / 2)} "Tipo de Muestra" segments`
	);
	...
	for (let i = 1; i < segments.length; i += 2) {
		...
		if (content.length > 50) {
			...
			console.log(
				`🧪 Sample block: ${cleanSampleType} (${content.length} chars)`
			);
		}
	}
	...
	if (blocks.length === 0) {
		console.log(
			'⚠️ No "Tipo de Muestra" blocks found, using entire text as SUERO'
		);
		blocks.push({
			sampleType: 'SUERO',
			content: text,
...
function filterMetadataContamination(text: string): string {
	...
	console.log(
		`🧹 Filtered ${lines.length - filteredLines.length} metadata lines`
	);
	return cleanedText;
}
 ...
 */
function stitchReferenceRanges(
	...
	console.log(
		`📋 Found ${resultsNeedingRanges.length} results missing reference ranges`
	);
	...
	for (const result of resultsNeedingRanges) {
		...
		for (let i = 0; i < lines.length; i++) {
			if (
				lines[i].includes(result.examen) &&
				lines[i].includes(String(result.resultado))
			) {
				resultLineIndex = i;
				break;
			}
			...
			for (const pattern of rangePatterns) {
				...
				for (const match of matches) {
					...
					if (potentialRange.length > 1 && !result.valorReferencia) {
						...
						attachedCount++;
						console.log(
							`🔗 Attached range "${potentialRange}" to ${result.examen}`
						);
						break;
					}
				}
...
function filterNoiseResults(
	...
	const filteredResults = results.filter((result) => {
		...
		if (result.resultType === 'numeric') {
			...
			if (isNaN(numericValue)) {
				console.log(
					`❌ Dropped non-numeric result: "${result.examen}" = "${result.resultado}"`
				);
				return false;
			}
		}
		...
		if (result.resultType === 'numeric' && !result.unidad) {
			...
			if (!allowedUnitless.some((allowed) => result.examen.includes(allowed))) {
				console.log(
					`❌ Dropped numeric result without unit: "${result.examen}" = ${result.resultado}`
				);
				return false;
			}
		}
		...
		if (result.confidence < 50) {
			console.log(
				`❌ Dropped low confidence result: "${result.examen}" (${result.confidence}%)`
			);
			return false;
		}
	...
	console.log(
		`🧹 Noise filtering: ${initialCount} → ${filteredResults.length} (removed ${initialCount - filteredResults.length})`
	);
 ...
 */
export function extractAllLabResults(
	text: string,
	pages?: string[]
): ComprehensiveLabResult[] {
	const results: ComprehensiveLabResult[] = [];
	...
	console.log(
		'🔍 Starting comprehensive lab extraction with sample type segmentation...'
	);
...
function extractNumericResults(
	...
	const valorReferenciaPattern =
		'(?:(?:Menor a|Mayor a|Hasta)\\s*[\\d.,<>\\s]+|[\\d.,]+(?:\\s*-\\s*[\\d.,]+)|Normal:?\\s*[\\s<>\\d.,]+|[\\d.,]+-[\\d.,]+)';

	const numericPatterns = [
		// Pattern for no space between name and result: H. TIROESTIMULANTE (TSH)11,040(μUI/mL)[ * ] 0,55-4,78
		new RegExp(
			`^([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\\s\\.()\\/-]{5,50})([\\d,]+(?:\\.\\d+)?)\\(([^)]+)\\)\\s*(\\[?\\s?\\*?\\s?\\]?)?\\s*(${valorReferenciaPattern})`,
			'gm'
		),
		// Constrained standard pattern: GLICEMIA EN AYUNO (BASAL) 269 (mg/dL) [ * ] 74-106
		new RegExp(
			`^([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\\s\\.()\\/-]{5,50})\\s+([\\d,]+(?:\\.\\d+)?)\\s+\\(([^)]+)\\)\\s*(\\[?\\s?\\*?\\s?\\]?)?\\s*(${valorReferenciaPattern})`,
			'gm'
		),

		// Compact pattern with constrained reference: CREATININA0,91(mg/dL) 0,55-1,02
		new RegExp(
			`([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\\s\\.()\\/-]{5,40})([\\d,]+(?:\\.\\d+)?)\\(([^)]+)\\)\\s*(\\[?\\s?\\*?\\s?\\]?)?\\s*(${valorReferenciaPattern})`,
			'g'
		),

		// Spaced pattern with constrained range: HEMOGLOBINA    14.2    (g/dL)    12.3-15.3
		new RegExp(
			`^([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\\s\\.()\\/-]{5,50})\\s{2,}([\\d,]+(?:\\.\\d+)?)\\s+\\(([^)]+)\\)\\s+(${valorReferenciaPattern})`,
			'gm'
		),
...
function extractTabularResults(
	...
	for (let i = 0; i < lines.length; i++) {
		...
		const tabularMatch = line.match(
			/^([A-ZÁÉÍÓÚÑ][A-ZÁÉÍÓÚÑ\s\.\(\)\/\-]{5,50})\s+(\d+(?:,\d+)?(?:\.\d+)?|\w+)\s*(?:\(([^)]+)\))?\s*(.*)$/
		);

		if (tabularMatch) {
			const [fullMatch, examen, resultado, unidad = null, rest = ''] =
				tabularMatch;
			...
			const labResult = createLabResult({
				examen: examen.trim(),
				resultado: isNaN(parseFloat(resultado.replace(',', '.')))
					? resultado
				...
				resultType: isNaN(parseFloat(resultado.replace(',', '.')))
					? 'qualitative'
					: 'numeric',
				confidence: 75,
				position: text.indexOf(fullMatch),
 ...
 */
function extractLabsWithSpecificFormats(
	text: string
): ComprehensiveLabResult[] {
	const results: ComprehensiveLabResult[] = [];
	const lines = text.split('\n');
	...
	for (let i = 0; i < lines.length; i++) {
		...
		for (const [labName, format] of Object.entries(CHILEAN_LAB_FORMATS)) {
			if (line.includes(labName)) {
				...
				const labResult = extractSpecificLabFormat(
					line,
					labName,
					format,
					i,
				...
				if (labResult) {
					console.log(
						`✅ Extracted specific: ${labResult.examen} = ${labResult.resultado}`
					);
					results.push(labResult);
				}
...
function extractSpecificLabFormat(
	...
	switch (format.type) {
		case 'numeric':
			return extractNumericLabFormat(
				line,
				labName,
				format,
				lineIndex,
		...
		case 'observation':
			return extractObservationLabFormat(
				line,
				labName,
				format,
				lineIndex,
		...
		case 'qualitative':
			return extractQualitativeLabFormat(
				line,
				labName,
				format,
				lineIndex,
		...
		case 'calculated':
			return extractCalculatedLabFormat(
				line,
				labName,
				format,
				lineIndex,
...
function extractObservationLabFormat(
	...
	const afterLabName = line
		.substring(line.indexOf(labName) + labName.length)
		.trim();
		...
		for (const expectedValue of format.expectedValues) {
			if (afterLabName.includes(expectedValue)) {
				return {
					...
					tipoMuestra: detectSampleType(allLines, lineIndex),
					isAbnormal:
						expectedValue !== 'Negativo' && expectedValue !== 'No reactivo',
					abnormalIndicator:
						expectedValue !== 'Negativo' && expectedValue !== 'No reactivo'
							? '[*]'
							: '',
...
function detectSampleType(allLines: string[], lineIndex: number): string {
	...
	for (let i = Math.max(0, lineIndex - 5); i < lineIndex; i++) {
		...
		if (line.includes('Tipo de Muestra : SANGRE TOTAL'))
...
function extractLabsByGroupStructure(
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (labMatch) {
			...
			const labResult = parse5ColumnStructure(
				labName,
				restOfLine,
				currentSampleType,
				healthMarkerLookup,
			...
			if (labResult) {
				console.log(
					`✅ Extracted 5-column: ${labResult.examen} = ${labResult.resultado}`
				);
				results.push(labResult);
			}
		}
		...
		if (
			...
			const embeddedResults = extractEmbeddedFromLine(
				line,
				currentSampleType,
				healthMarkerLookup,
				i
...
function extractEmbeddedMultiResults(
	...
	for (const pattern of embeddedPatterns) {
		let match;
		while ((match = pattern.exec(text)) !== null) {
			const [fullMatch, examen, resultado, unidad, valorReferencia = ''] =
				match;
			...
			let cleanResultado: number | string = parseFloat(
				resultado.replace(',', '.')
			);
...
function cleanContaminatedNormalRanges(text: string): string {
	...
	for (const pattern of contaminatedPatterns) {
		cleanedText = cleanedText.replace(
			pattern,
			(match, labName, result, unit, contamination) => {
				...
				const rangePart = contamination.match(
					/^([^R]*?)(?=RUT|Fecha|Método|$)/
				);
				if (rangePart) {
 ...
 */
function cleanHeaderFooterContamination(
	text: string,
	pages?: string[]
): string {
	let cleanedText = text;
	...
	console.log(
		`🧹 Cleaned ${text.length - cleanedText.length} characters of header/footer contamination`
	);
...
function extractSampleType(text: string): string {
	...
	for (const sampleType of sampleTypes) {
		if (
			text.includes(`Tipo de Muestra : ${sampleType}`) ||
...
function isAlreadyExtracted(
	...
	return results.some(
		(r) => r.examen.toUpperCase().trim() === normalizedExamen
	);
}
...
function createLabResult(params: {
	...
	if (typeof params.resultado === 'number' && params.valorReferencia) {
		const severityResult = calculateClinicalSeverity(
			params.resultado,
			params.valorReferencia,
			params.unidad || ''
		);
...
function calculateClinicalSeverity(
	...
	const lessThanMatch = normalizedRange.match(
		/(?:Menor\s+a|<)\s*(\d+(?:\.\d+)?)/
	);
	if (lessThanMatch) {
