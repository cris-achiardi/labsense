...
export default function PatientDetailPage({ params }: PatientDetailPageProps) {
	...
	const getLabUnit = (markerType: string): string => {
		...
		if (marker.includes('glicemia') || marker.includes('glucosa'))
			return 'mg/dL';
		...
		if (
			marker.includes('sodio') ||
			marker.includes('potasio') ||
			marker.includes('cloro')
		)
			return 'mEq/L';
		if (marker.includes('creatinina')) return 'mg/dL';
		if (marker.includes('colesterol') || marker.includes('trigliceridos'))
			return 'mg/dL';
		if (marker.includes('hemoglobina') || marker.includes('hematocrito'))
			return 'g/dL';
	...
	const getNormalRange = (markerType: string): string => {
		...
		if (marker.includes('nitrogeno') || marker.includes('urea'))
			return '9 - 23';
