/**
 * Demo Complete Lab Processing API
 * Based on the original process-complete-lab but without authentication
 * Allows real PDF processing for demo/hackathon presentations
 */
	...
	try {
		...
		if (extractionResult.metadata.folio) {
			...
			if (allReports && allReports.length > 0) {
				...
				overwrite = true;
				console.log(`🔄 Demo: Overwriting existing report for folio ${extractionResult.metadata.folio}`);
			}
		}
		...
		const { data: labReportId, error: storeError } = await supabaseAdmin.rpc(
			'store_complete_lab_extraction',
			{
				...
				p_lab_results: extractionResult.labResults.map((result, index) => {
					console.log(
						`🔍 Demo: Processing result ${index + 1}/${extractionResult.labResults.length}:`,
						{
				...
				p_uploaded_by: 'demo@labsense.ai', // Demo user identifier
				p_file_name: file.name,
				p_file_path: `demo-lab-pdfs/${Date.now()}-${file.name}`, // Demo file path
		...
		if (storeError) {
			console.error('Demo database storage error:', storeError);
			return NextResponse.json(
				{
		...
		if (fetchError) {
			console.error('Demo: Error fetching stored report:', fetchError);
		}

		// Log successful processing
		console.log('Demo complete lab processing successful:', {
			folio: extractionResult.metadata.folio,
		...
		return NextResponse.json({
			success: true,
			message: 'Examen de laboratorio procesado exitosamente (Demo)',
			data: {
				labReportId,
				folio: extractionResult.metadata.folio,
				patient: {
		...
		console.error('Demo complete lab processing API error:', error);
		return NextResponse.json(
			{
...
export async function GET() {
	return NextResponse.json({
		message: 'Demo Complete Lab Processing API',
		description:
			'Processes and stores complete lab extraction results without authentication for demo purposes',
		method: 'POST',
		parameters: {
